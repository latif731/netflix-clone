...
const importMovies = asyncHandler(async (req, res) => {
    ...
    // them we insert all movies from MoviesData
...
const getMovies = asyncHandler(async(req, res) => {
    try{
        // filter movie by category, time, language, rate, year and search
...
const createMovieReview = asyncHandler(async(req, res) => {
    ...
    try{
        ...
        if(movie){
            ...
            // calculate th new rate
            const totalReviews = movie.reviews.length
            const totalRating = movie.reviews.reduce((acc, reviews) =>  reviews.rating + acc, 0);
            movie.rate = totalRating / totalReviews
            // save the movies in database   
            await movie.save()
            // send the new movie to database
            res.status(201).json({
...
const updateMovie = asyncHandler(async(req, res) => {
    try{
        ...
        if(movie){
            ...
            // send the updated movie to th client
...
const deleteAllMovies = asyncHandler(async(req, res) => {
    try{
        // delete all movie
        await Movie.deleteMany({});
        res.json({message:"all movies have been deleted"});
    }catch(error){
        res.status(400).json({message: error.message})
    }
})
