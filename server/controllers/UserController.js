...
// import { Error } from "mongoose""Invalid email or password";
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (user && (await bcrypt.compare(password, user.password))) {
      ...
      // res.status(201);
      // throw new Error(`welcome back ${fullName}`)
      // if user mot found or password not match send error message
    } else {
      res.status(401);
...
const updateUserProfile = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      res.json({
        _id: updatedUser._id,
        fulllName: updatedUser.fullName,
...
const deleteUserProfile = asyncHandler(async (req, res) => {
  try {
    // find user in DB
    const user = await User.findById(req.user._id);
    // if use exists delete user from DB
    if (user) {
...
const deleteLikedMovies = asyncHandler(async (req, res) => {
  try {
    // find user in DB
    const user = await User.findById(req.user._id);
    // if user exista delete all liked movies and save it in DB
    if (user) {
      user.likedMovies = [];
      await user.save();
